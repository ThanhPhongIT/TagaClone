...
export class SocketService {
  private socket$: any;
  private accessToken: any = this.getObject('tokenData');
  ...
  constructor() {
    ...
    // this.accessToken = this.getObject('access_token');
  }
  getObject(key: string) {
    return JSON.parse(localStorage.getItem(key) || '').idToken.jwtToken;
  }
  /**
  ...
  public connect(cfg: { reconnect: boolean } = { reconnect: false }): void {
    if (!this.socket$ || this.socket$.closed) {
      console.log(`${WS_ENDPOINT}${this.accessToken}`);
      
      this.socket$ = webSocket(`${WS_ENDPOINT}${this.accessToken}`);
      this.socket$.subscribe(
        (msg: any) => console.log('message received: ' + msg),
        (err: any) => console.log(err),
        () => console.log('complete')
      );
      // this.socket$.next(JSON.stringify({ op: 'hello' }));
  ...
  private getNewWebSocket() {
    return webSocket({
      url: `${WS_ENDPOINT}${this.accessToken}`,
      deserializer: (msg) => {
        try {
